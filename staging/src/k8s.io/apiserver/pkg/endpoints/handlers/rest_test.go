...
func TestPatchAnonymousField(t *testing.T) {
	...
	err := strategicPatchObject(defaulter, original, []byte(patch), actual, &testPatchType{}, nil)
	if err != nil {
...
func TestPatchCustomResource(t *testing.T) {
	...
	err := strategicPatchObject(defaulter, original, []byte(patch), actual, &unstructured.Unstructured{}, nil)
	if !apierrors.IsBadRequest(err) {
		t.Errorf("expected HTTP status: BadRequest, got: %#v", apierrors.ReasonForError(err))
	}
...
func TestNumberConversion(t *testing.T) {
	...
	err := strategicPatchObject(defaulter, currentVersionedObject, patchJS, versionedObjToUpdate, schemaReferenceObj, nil)
	if err != nil {
		t.Fatal(err)
	}
